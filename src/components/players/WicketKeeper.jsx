...
const WicketKeeper = ({ position = [0, 0, 0] }) => {
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      meshRef.current.position.y = -0.5 + Math.sin(state.clock.elapsedTime * 1.5) * 0.05;
    }
  });
    ...
    <group position={position}>
      <group ref={meshRef}>
        ...
        <mesh position={[0, 1.2, 0]}>
          ...
          <meshLambertMaterial color="#000000ff" />
        </mesh>
        ...
        {/* Arms - ready to catch */}
        <mesh position={[-0.3, 0.7, 0.3]} rotation={[-0.8, 0, -0.3]}>
        ...
        {/* Legs - crouched */}
        <mesh position={[-0.1, 0.0, 0]} rotation={[-0.3, 0, 0]}>
        ...
        {/* Pads */}
        <mesh position={[-0.1, 0.0, 0.1]} rotation={[-0.3, 0, 0]}>
